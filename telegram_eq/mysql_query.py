...
sql_conf = config["mysql"]          # словарь конфигов для mysql
...
class MySQL():      
    ...
    def create_user(self, tg_id):
        connect = self.connect()
        sql = f"INSERT INTO users(tg_user_id, fio, phone, fio_reg, phone_reg, last_bot_use_unixtime, LastSelectMFC, LastSelectGroup, LastSelectUsluga, LastSelectDate, LastColOrder, LastStep, LastIdMenu, LastComment) VALUES ('{tg_id}','','','no','no','{int(datetime.datetime.now().timestamp() * 1000)}','','','','','','','','');"
        cursor = connect.cursor()
        try:
            cursor.execute(sql)
            connect.commit()
    ...
    def get_col_tickets(self, id):
        connect = self.connect()
        sql = f"SELECT COUNT(*) AS count FROM mfc_tg_bot.tickets WHERE create_unixtime >= UNIX_TIMESTAMP(CURDATE()) * 1000 AND create_unixtime < UNIX_TIMESTAMP(NOW()) * 1000 and tg_id_owner='{id}';"
        cursor = connect.cursor()
        result = False
        try:
            cursor.execute(sql)
...
async def update_status_ticket(loop, tg_id, text, pin):
    pool = await aiomysql.create_pool(host='192.168.137.88', port=3306,
                                      user='root', password='Suit1234',
                                      db='mfc_tg_bot', loop=loop)
...
sql_init = MySQL('192.168.137.88', 'mfc_tg_bot', 'root', '')    # инициализируем класс для подключения к базе
